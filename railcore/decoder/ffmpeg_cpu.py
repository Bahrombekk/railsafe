...
class FFMPEGCPUDecoder(VideoDecoder):
    """FFMPEG CPU decoder (fallback)"""
    ...
    def _open(self) -> bool:
        ...
        try:
            ...
            if self.cap.isOpened():
                ...
                logger.info(f"FFMPEG CPU decoder ochildi: {self.source}")
                return True
            else:
                logger.warning(f"FFMPEG CPU decoder ochilmadi: {self.source}")
                return False
        except Exception as e:
            logger.error(f"FFMPEG CPU xato: {e}")
            return False
